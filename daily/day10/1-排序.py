...
class Sort:
    ...
    def count_sort(self):
        ...
        count = [0] * (max_value - min_value + 1)  # 记录所有值出现次数的列表

        for i in self.arr:
            count[i - min_value] += 1
        # 回填
        k = 0
        for val in range(min_value, max_value + 1):  # 遍历数值范围
            for j in range(count[val - min_value]):  # 对应数值出现了多少次，我们就回填多少次
                self.arr[k] = val
                k += 1

    def statistics_sort_time(self, sort_method=None, *args):
        """
        统计不同排序算法时间
        :return:
        """
        ...
        if sort_method:
            sort_method(*args)
            # 比如调用了mergesort ，先解析*args拆分元组，然后把元组作为参数传入mergesort( parm1 , parm2 )
        end = time.time()
        print(f'排序使用时间{end - start}')
...
if __name__ == '__main__':
    ...
    # 函数回调 statistics_sort_time( 函数 ， 参数 ）
    my_sort.statistics_sort_time(my_sort.merge_sort,0,5000)
# my_sort.statistics_sort_time(my_sort.merge_sort,0,100)  排序使用时间0.0070018768310546875
# my_sort.statistics_sort_time(my_sort.merge_sort,0,5000) 排序使用时间0.6561744213104248